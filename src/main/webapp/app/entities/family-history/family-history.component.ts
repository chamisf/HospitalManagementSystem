...
currentAccount: any;
    ...
    loadAll() {
        this.familyHistoryService.query({
            page: this.page - 1,
    ...
    transition() {
        this.router.navigate(['/family-history'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/family-history', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: FamilyHistory) {
        return item.id;
    }
    registerChangeInFamilyHistories() {
        this.eventSubscriber = this.eventManager.subscribe('familyHistoryListModification', (response) => this.loadAll());
    }
